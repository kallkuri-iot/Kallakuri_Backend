...
/**
 * @desc    Dispatch sales inquiry (Godown Incharge)
 * @route   PATCH /api/mobile/sales-inquiries/:id/dispatch
 * @access  Private (Godown Incharge, Admin)
 */
exports.dispatchSalesInquiry = async (req, res, next) => {
  try {
    // Check if user has Godown Incharge or Admin role
    if (req.user.role !== 'Godown Incharge' && req.user.role !== 'Admin') {
      return res.status(403).json({
        success: false,
        error: 'Access denied. Only Godown Incharge and Admins can dispatch sales inquiries'
      });
    }

    const { dispatchDate, dispatchDetails, trackingNumber } = req.body;
    ...
    // Check if inquiry is in the correct state
    if (salesInquiry.status !== 'Pending' && salesInquiry.status !== 'Commented') {
      return res.status(400).json({
        success: false,
        error: 'Only pending or commented inquiries can be dispatched'
      });
    }

    // Update sales inquiry with dispatch details
    salesInquiry.status = 'Dispatched';
    salesInquiry.dispatchDate = dispatchDate || new Date();
    salesInquiry.dispatchDetails = dispatchDetails;
    salesInquiry.trackingNumber = trackingNumber;
    salesInquiry.dispatchedBy = req.user.id;
    salesInquiry.dispatchedAt = new Date();

    await salesInquiry.save();

    // Log staff activity
    await StaffActivity.create({
      staffId: req.user.id,
      activityType: 'Dispatch',
      details: `Dispatched sales inquiry for ${salesInquiry.distributorId.name}`,
      status: 'Completed',
      relatedId: salesInquiry._id,
      onModel: 'SalesInquiry'
    });
    ...
    logger.error(`Error in dispatchSalesInquiry controller: ${error.message}`);
    next(error);
  }
};
...
/**
 * @desc    Get all sales inquiries for managers and godown incharge (shows all staff inquiries)
 * @route   GET /api/mobile/sales-inquiries/all-inquiries
 * @access  Private (Mid-Level Manager, Admin, Godown Incharge)
 */
exports.getAllSalesInquiriesForManager = async (req, res, next) => {
  try {
    const { status } = req.query;
    
    // Check if user has manager, admin, or godown incharge role
    if (req.user.role !== 'Mid-Level Manager' && req.user.role !== 'Admin' && req.user.role !== 'Godown Incharge') {
      return res.status(403).json({
        success: false,
        error: 'Access denied. Only managers, admins, and godown incharge can view all sales inquiries'
      });
    }
    
    // Build query - show all inquiries for managers and godown incharge
    let query = {};
    ...
    logger.error(`Error in getAllSalesInquiriesForManager controller: ${error.message}`);
    next(error);
  }
};
